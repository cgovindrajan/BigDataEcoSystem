...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    # Output:                                                         
    # - resultVector: feature vector for the sentence    
    
    resultVector = np.zeros((wordVectors.shape[1],))
    
    ### YOUR CODE HERE
    #raise NotImplementedError
    array = np.fromiter( (tokens[word] for word in sentence), dtype='int')
    resultVector = np.mean(wordVectors[array], axis=0)
    ### END YOUR CODE
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    probCopy = prob
    probCopy[np.arange(N), labels] -= 1
    probCopy = probCopy/N

    # probCopy is the gradient associated with the loss (softmax layer only)
    grad = np.dot(features.T, probCopy)


    #backprop the weights
    gradInc = regularization * weights
    grad = grad + gradInc
    ### END YOUR CODE
